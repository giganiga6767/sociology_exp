...
export default function Dashboard() {
  ...
  const [selectedChannel, setSelectedChannel] = useState<string>("Global_Square");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch raw simulation state
        try {
          const stateRes = await fetch("http://127.0.0.1:8000/api/state");
          if (stateRes.ok) {
            const stateJson = await stateRes.json();
            setSimState(stateJson);
          }
        } catch (stateErr) {
          console.error("Could not fetch state API", stateErr);
        }
  ...
  return (
    <div className="min-h-screen bg-[#0B0F19] text-gray-100 p-8 font-sans">
      ...
      {/* Dynamic Chat Feed Panel */}
      <div className="bg-[#111827]/40 border border-gray-800/80 rounded-3xl p-6 mb-8 backdrop-blur-md">
        <h3 className="text-base font-bold text-gray-200 mb-4 flex items-center justify-between">
          <span className="flex items-center gap-2">💬 Live Dialogue Chat Feed</span>
          {simState && (
            <span className="text-xs text-cyan-400 font-mono bg-cyan-950/40 border border-cyan-800/50 px-2.5 py-0.5 rounded-full">
              ROUND: {simState.round_count} | TURNS: {simState.turn_count}
            </span>
          )}
        </h3>
        ...
        <div className="flex gap-2 border-b border-gray-800 pb-3 mb-6">
          {["Global_Square", "Tier2_Family", "Cosmopolitan_Family"].map((chan) => (
            <button
              key={chan}
              ...
              className={`px-4 py-1.5 rounded-xl text-sm font-semibold transition border ${
                selectedChannel === chan
                  ...
                  : "bg-transparent text-gray-400 border-transparent hover:bg-gray-800/20"
              }`}
            >
              {chan.replace("_", " ")}
            </button>
        ...
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
          {simState?.channels?.[selectedChannel]?.length > 0 ? (
            simState.channels[selectedChannel].map((msg: any, idx: number) => {
              const isObserver = msg.sender_id === "God_Agent" || msg.sender_name === "Observer";
              const isParent = msg.sender_id.includes("Parent");
              const isStudent = msg.sender_id.includes("Student");
              const isSystem = msg.sender_id === "system";

              return (
                <div key={idx} className={`flex flex-col ${isSystem ? "items-center" : "items-start"} max-w-3xl ${isSystem ? "mx-auto w-full" : ""}`}>
                  {isSystem ? (
                    <div className="my-4 text-center px-4 py-2 bg-yellow-950/20 border border-yellow-800/30 rounded-xl text-yellow-300 text-xs font-mono">
                      {msg.content}
                    </div>
                  ) : (
                    <div className={`w-full p-4 rounded-2xl border ${
                      isObserver
                        ...
                        : isParent
                        ? "bg-[#1f1e2e]/40 border-violet-800/20 text-violet-100"
                        : "bg-[#111c2e]/40 border-cyan-800/20 text-cyan-100"
                    }`}>
                      {/* Message Sender and Meta info */}
                      <div className="flex justify-between items-center mb-2">
                        <span className={`font-bold text-xs uppercase tracking-wider ${
                          isObserver ? "text-gray-400" : isParent ? "text-violet-400" : "text-cyan-400"
                        }`}>
                          {msg.sender_name} {isObserver ? "(OBSERVER)" : `(${msg.emotional_state || "composed"})`}
                        </span>
                      ...
                      {/* Hidden CoT Reasoning inspector */}
                      {msg.thinking && msg.thinking !== "No structured thinking block detected." && (
                        <div className="mt-3 border-t border-gray-800/60 pt-3">
                          <button
                            onClick={() => setExpandedThinking(prev => ({ ...prev, [idx]: !prev[idx] }))}
                            className="text-[11px] font-semibold text-gray-400 hover:text-cyan-400 flex items-center gap-1 transition"
                          >
                            {expandedThinking[idx] ? "▼ Hide Inner Thought Process" : "▶ Show Inner Thought Process"}
                          </button>
                          {expandedThinking[idx] && (
                            <div className="mt-2 p-3 bg-black/40 border border-gray-800 rounded-xl text-xs text-gray-400 font-mono leading-relaxed whitespace-pre-line">
                              {msg.thinking}
                            </div>
                          )}
                        </div>
            ...
            <div className="text-center py-12 text-gray-500 text-sm">
              No conversation logs available in this channel.
            </div>
          )}
        </div>
      </div>
